...
if ((_max_goats - _current_goats) > 0) then {
    ...
    for "_i" from 1 to _goats_to_spawn do {
        _goats = createAgent ["Goat_random_F", getPos SolidSnake, [], 5, "CAN_COLLIDE"];
    };

    [SolidSnake, [selectRandom ["invisible", "waiting", "dna", "cheeks", "metalGear"], 100, 1]] remoteExec ["say3D", ([0, -2] select isDedicated), true];
} else {
